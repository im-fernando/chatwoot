class Whatsapp::Providers::WhatsappCloudService < Whatsapp::Providers::BaseService
  ...
  def send_attachment_message(phone_number, message)
    ...
    if %w[audio sticker].include?(type.to_s)
      return nil if cloud_send_signed_text_preamble(phone_number, message) == :fail
    end
  ...
  def build_attachment_content(type, attachment, message)
    ...
    type_content['caption'] = Whatsapp::OutgoingSignature.body_for_whatsapp(message) unless %w[audio sticker].include?(type)
  ...
  def cloud_send_signed_text_preamble(phone_number, message)
